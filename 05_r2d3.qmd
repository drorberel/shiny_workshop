...
This document is based on example from
...
It calls two .JS functions:

<https://github.com/sol-eng/db-dashboard/blob/main/local/col_plot.js>

<https://github.com/sol-eng/db-dashboard/blob/main/local/bar_plot.js>

![](images/r2d3.png)

A few things worth noting about the architecture: The `base_flights()`
reactive is the central spine. Both `selectInputs` feed into it, and
every output - value boxes, D3 plots, and detail tabs - draws from it.
This is the standard Shiny "single reactive source of truth" pattern.

**The r2d3 bridge is a two-way street.** `renderD3` pushes R data
(already aggregated by dplyr) down to `col_plot.js` and `bar_plot.js`.
Those JS files then push click events *back* into Shiny as
`inputcolumnclicked‘and‘inputcolumn_clicked` and
`input columnc​licked‘and‘inputbar_clicked` - that's the part that's easy
to miss.

The JS files aren't just renderers; they're also event emitters.
`col_plot.js` has dual behavior depending on whether
`input$month == 99`.

If "All Year" is selected, a bar click calls `updateSelectInput` to
drill into that month. If a specific month is already selected, it calls
`appendTab` instead - different branch, same click event. get_details()
is a plain function (not a reactive), shared by both `observeEvent`
handlers. It gets called lazily inside `renderDataTable` inside the
dynamically appended tab - so it only runs when that tab is viewed.
